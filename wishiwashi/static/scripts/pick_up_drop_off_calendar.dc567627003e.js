$(document.body).ready(function() {
    ...
    bind_form_submit_events = function(_prefix) {
        ...
        $(_prefix + " .time-slot").click(function(event){
            if(!$(this).hasClass('unavailable-slot')) {
                ...
                $(this).after('<img src="' + check_png +'" width="39" height="39" alt="" class="selected-slot-checkmark">')
    ...
    first_selected_day_offset = function() {
        ...
        // selectable times
        if(parseInt(selected_day) > 0) {
            return parseInt(selected_day);
        }
    ...
    build_day_selectors = function() {
        ...
        $('#mobile_next').click(function(event) {
            ...
            if(day_offset < calendar_grid.length - 2) {
                day_offset ++;
                draw_day(day_offset);
                $('#mobile_previous').prop('disabled', false);
        ...
        if(day_offset == first_day_offset) {
            $('#mobile_previous').prop('disabled', true);
        }

        if(day_offset >= calendar_grid.length - 2) {
            $('#mobile_next').prop('disabled', true);
        }
    },
    
    // This is only used by the desktop version
    week_offset = parseInt(selected_week),
    ...
    build_week_selectors = function() {
        $('#desktop_previous').click(function(event) {
            ...
            if(week_offset > 0) {
                ...
                $('#desktop_next').prop('disabled', false);
        ...
        $('#desktop_next').click(function(event) {
            ...
            if(week_offset < (calendar_grid.length / 6) - 1) {
                week_offset ++;
                draw_week(week_offset);
                $('#desktop_previous').prop('disabled', false);
        ...
        if(week_offset == 0) {
            $('#desktop_previous').prop('disabled', true);
        }

        if(week_offset >= (calendar_grid.length / 6) - 1) {
            $('#desktop_next').prop('disabled', true);
        }
    },
    ...
    if((calendar_grid.length/6) == 1) {
        $('#desktop_next').prop('disabled', true);
    }else if(week_offset < ((calendar_grid.length / 6) - 1)){
        $('#desktop_next').prop('disabled', false);
    }

    if(typeof selected_date !== 'undefined' && typeof selected_hour !== 'undefined'){
